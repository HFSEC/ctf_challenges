  ...
  <body>
    ...
    <form action="#" method="post">
      ...
      <td>
        ...
        $a=mysql_query("SELECT * FROM user WHERE username = '$user' AND password = '$pass'",$link);
        $b=mysql_fetch_array($a);
        if($b) {
            echo header('Location: ./PTFPZAAALBIHMAHRXZYUUPIPQJSEPRAG.html');
        }
        else{
          if($link)
          {
      ...
      <td class="box1">提示：</td>
      <td>
        FLAG为admin的密码（是直接从数据库中读出的数据，不需要再次破解md5）。
        <br/>
        提交的时候记得补充为HFCTF{******}的格式
      </td>
      </tr>
